  ...
  const { data: value } = useQuery({
    ...
    queryFn: async () => {
      const resultsReq = await fetch(
        `http://127.0.0.1:7777/apii/top-tracks/${time_range}`,
        {
          credentials: "include",
        },
      );
...
export function TopArtists({ range }: { range: string }) {
  const { data: artistsArray } = useQuery({
    ...
    queryFn: async () => {
      const resultsReq = await fetch(
        `http://127.0.0.1:7777/apii/top-artists/${range}`,
        {
          credentials: "include",
        },
      );
...
export function TopItemsImage({
  ...
  const { data: profileDetails } = useQuery({
    queryKey: ["profile"],
    queryFn: async () => {
      const profile = await fetch("http://127.0.0.1:7777/apii/profile", {
        credentials: "include",
      });
...
function ArtistsImageView({ time_range }: { time_range: string }) {
  const { data: artists } = useQuery({
    ...
    queryFn: async () => {
      const resultsReq = await fetch(
        `http://127.0.0.1:7777/apii/top-artists/${time_range}`,
        {
          credentials: "include",
        },
      );
